...
void insercao(struct cadastro* contatos){
...
void impressao(struct cadastro* contatos){ //imprime o contato cadastrado em ordem e seguindo uma formatação
...
void remocao(struct cadastro *contatos){
...
void busca(struct cadastro* contatos){
...
int identificaoperacao(char c,struct cadastro* contatos){
    scanf("\n");
    if (c == 'i'){
        insercao(contatos);
        return 1;
...
struct cadastro* criacontato(void){
    static int execi=0;
    static struct cadastro contatos[1000];
    if (execi==0){
...
int main(void){
    ...
    while(identificaoperacao(getchar(),criacontato()) == 1){
    }
    printf("\n");
    return 0;
}